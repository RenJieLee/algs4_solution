public class Solution24 {
	public static void main(String[] args) {
		...
		for (int N = 125; N > 0; N += N) {
			double weightedUFTime = 0.0;
			double compressUFTime = 0.0;
			for (int i = 0; i < T; i++) {
				double tmp = System.currentTimeMillis();
				ErdosRenyi.countWeightedQuickUnionUF(N);
				weightedUFTime += (System.currentTimeMillis() - tmp);
				tmp = System.currentTimeMillis();
				ErdosRenyi.countCompressWeightedQuickUnionUF(N);
				compressUFTime += (System.currentTimeMillis() - tmp);
			}
			StdOut.printf("%d %.5f\n", N, weightedUFTime / compressUFTime);
		}
	}
}
...
class ErdosRenyi {
	...
	public static int countCompressWeightedQuickUnionUF(int N) {
		//int N = Integer.parseInt(args[0]);
...
class CompressWeightedQuickUnion {
	private int[] id;
	private int[] sz;
	private int count;
