public class Solution22 {
	public static void main(String[] args) {
		...
		for (int i = 0; i < T; i++) {
			...
			ErdosRenyi.countWeightedQuickUnionUF(125);
			pre += (System.currentTimeMillis() - tmp);
		}
		for (int N = 250; N > 0; N += N) {
			...
			for (int i = 0; i < T; i++) {
				double tmp = System.currentTimeMillis();
				aver += ErdosRenyi.countWeightedQuickUnionUF(N);
				res += (System.currentTimeMillis() - tmp);
			}
			StdOut.println(N + " " + (aver / 10) + " " + res / pre);
			pre = res;
		}
	}
}
