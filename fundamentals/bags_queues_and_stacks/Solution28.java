...
public class Solution28<Item> implements Iterable<Item> {
	...
	public static boolean find(Solution28<String> list, String string) {
		for (String s : list)
			if (s.equals(string))
				return true;
		return false;
	}
	public static void remove(Solution28<String> list, String string) {
	...
	public static void main(String[] args) {
		Solution28<String> list = new Solution28<String>();
		String[] temp = ("f u f c k y f f f o u f").split(" ");
		for (String s : temp) {
			list.add(s);
		}
		StdOut.println(recurMax(list.getFirst()));
		remove(list, "f");
		for (String s : list)
			StdOut.print(s);
	}
